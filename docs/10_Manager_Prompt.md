...
When Path B or Path C applies, route to **`sales`** immediately. Sales continues **in the same chat** with packages and pricing; Discovery does not answer again.
...
**Override:** Path B and Path C **override** the generic rule below that would keep "thanks" or "when" on discovery — see Edge Cases.
...
- **Greeting or small talk:** Route to the currently active agent. If first message, route to discovery. **Exception:** If Path B or Path C applies (Discovery qualified + last assistant forward step + user short reply like "thanks" / "when" / "ok"), route to **`sales`** — do not treat those as generic small talk that keeps discovery.
